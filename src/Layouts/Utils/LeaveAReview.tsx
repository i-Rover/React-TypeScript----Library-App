            ...
            <StarsReview rating={starInput} size={32} />
            {
                ...
                <form method="POST" action="#">
                    ...
                    <textarea className="form-control" id="submitReviewDescription" rows={3}
                        placeholder="Optional..." onChange={e => setReviewDescription(e.target.value)}></textarea>
                    <div className="d-flex flex-row mb-3">
                        {
                            props.isSubmitReview ?
                                <button className="btn btn-primary mt-3" type="button" disabled>
                                    <span className="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span>
                                    Loading...
                                </button>
                                :
                                <button onClick={() => props.submitReview(starInput, reviewDescription)} type="button" className="btn btn-primary mt-3">Submit Review</button>
                        }
                    </div>
                </form>
            }
